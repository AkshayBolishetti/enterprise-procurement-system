...
export const Notifications = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 h-full flex flex-col">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-[var(--card)] p-6 rounded-3xl shadow-sm border border-[var(--input)] shrink-0">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-[var(--foreground)]">Notifications</h1>
          <p className="text-sm text-[var(--secondary-foreground)] mt-1">Stay updated on new orders, payments, and system alerts.</p>
        </div>
      ...
      <div className="bg-[var(--card)] rounded-3xl shadow-sm border border-[var(--input)] divide-y divide-[var(--input)] overflow-y-auto flex-1">
        {loading ? (
          <div className="p-8 text-center text-[var(--muted)]">Loading notifications...</div>
        ) : notifications.length === 0 ? (
          <div className="p-16 text-center flex flex-col items-center justify-center text-[var(--muted)] h-full">
          ...
          notifications.map(notif => (
            <div key={notif.id} className={`p-6 flex gap-4 transition-colors hover:bg-[var(--secondary)]/50 ${notif.isRead ? 'bg-[var(--card)]' : 'bg-[var(--primary)]/5'}`}>
              ...
              <div>
                ...
                <p className="text-[var(--secondary-foreground)] mt-1 text-sm">{notif.message}</p>
                <p className="text-xs text-[var(--muted)] mt-2 font-mono">{new Date(notif.createdAt || notif.timestamp).toLocaleString()}</p>
              </div>
            </div>
          ))
        )}
