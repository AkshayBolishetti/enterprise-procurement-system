...
import { 
  ...
  FileText, Package, Truck, ShieldCheck, Users, Bell
} from 'lucide-react';
...
export const AdminLayout = () => {
  ...
  const navigation = [
    ...
    { name: 'Payments', href: '/admin/payments', icon: Truck },
  ...
  return (
    <div className="min-h-screen bg-[var(--background)] flex">
      ...
      {/* Sidebar */}
      <aside className="w-64 bg-[var(--card)] border-r border-[var(--input)] flex-shrink-0 flex flex-col hidden md:flex h-screen sticky top-0 z-20">
        <div className="h-20 flex items-center gap-3 px-8 border-b border-transparent">
          ...
          <span className="font-bold text-xl tracking-tight text-[var(--foreground)]">Procurement System</span>
        </div>
        ...
        <div className="p-4 border-t border-[var(--input)]">
          <div className="px-4 py-3 mb-2">
            ...
            <p className="text-xs text-[var(--muted)] capitalize">{user?.role?.toLowerCase() || 'admin'}</p>
          </div>
          <Link
            to="/admin/settings"
